...
async function handleRequest(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const sessionIdMatch = pathname.match(/^\/api\/timer\/sessions\/([^\/]+)$/);
    const sessionId = sessionIdMatch ? sessionIdMatch[1] : null;

    if (pathname === '/api/timer/sessions' && req.method === 'GET') {
      const { GET } = await import('./sessions/route');
      response = await GET(mockRequest as any);
    } else if (pathname === '/api/timer/sessions' && req.method === 'POST') {
      ...
      response = await GET(mockRequest as any, { params: Promise.resolve({ id: sessionId }) });
    } else if (sessionId && req.method === 'PATCH') {
      const { PATCH } = await import('./sessions/[id]/route');
      response = await PATCH(mockRequest as any, { params: Promise.resolve({ id: sessionId }) });
    } else if (sessionId && req.method === 'DELETE') {
      const { DELETE } = await import('./sessions/[id]/route');
      response = await DELETE(mockRequest as any, { params: Promise.resolve({ id: sessionId }) });
    } else if (pathname === '/api/timer/settings' && req.method === 'GET') {
      const { GET } = await import('./settings/route');
      response = await GET(mockRequest as any);
    } else if (pathname === '/api/timer/settings' && req.method === 'PATCH') {
      const { PATCH } = await import('./settings/route');
      response = await PATCH(mockRequest as any);
    } else if (pathname === '/api/timer/stats' && req.method === 'GET') {
